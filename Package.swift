...
let package = Package(
    ...
    targets: [
        ...
        // P2-T5: ml/eval/cer.py가 호출하는 픽스처 전사 CLI(wide·narrow8k 두 대역).
        .executableTarget(
            name: "TranscribeFixtures",
            dependencies: ["Capture", "Preprocess", "STT"]
        ),
        // P3-T7: ml/eval/run.py가 호출하는 STT→Detection→AlertPolicy 전 구간 평가 CLI.
        .executableTarget(
